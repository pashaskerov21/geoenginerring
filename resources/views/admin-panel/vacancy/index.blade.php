            ...
            <table class="table table-striped table-centered mb-0">
                ...
                <tbody id="sortable-tbody">
                    @foreach ($vacancies as $vacancy)
                        <tr id="sort_{{ $vacancy->id }}">
                            <td>{{ $loop->index + 1 }}</td>
                            <td>{{ $vacancy->getTranslate->first()->title }}</td>
                            <td class="table-action d-flex" style="height: 70px">
                                <a href="{{route('admin.vacancy.edit', $vacancy->id)}}" class="action-icon me-1"> <i class="mdi mdi-square-edit-outline"></i></a>
        ...
        <script>
            $('#sortable-tbody').sortable({
                ...
                update: function() {
                    ...
                    $.get("{{ route('admin.vacancy.sort') }}?"+siralama, function(response) {});
                }
            });
        </script>
    @endpush
